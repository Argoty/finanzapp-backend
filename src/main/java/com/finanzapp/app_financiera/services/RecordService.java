...
public class RecordService {
    ...
    public List<MontoPeriodoDTO> obtenerBucketsPorPeriodo(int userId, String lastPeriod) {
        ...
        switch (lastPeriod.toLowerCase()) {
            case "1 semana": start = today.minusDays(6); buckets = 7; break;
            case "1 mes":    start = today.minusMonths(1); buckets = 4; break;
            case "3 meses":  start = today.minusMonths(3); buckets = 6; break;
            case "6 meses":  start = today.minusMonths(6); buckets = 6; break;
            case "1 año":    start = today.minusYears(1); buckets = 6; break;
    ...
    private LocalDateTime getDateLimit(String period) {
        ...
        return switch (period.toLowerCase()) {
            case "1 semana" -> now.minusWeeks(1);
            case "1 mes"    -> now.minusMonths(1);
            case "3 meses"  -> now.minusMonths(3);
            case "6 meses"  -> now.minusMonths(6);
            case "1 año"    -> now.minusYears(1);
            default -> null;
        };
    }
    ...
    private List<MontoPeriodoDTO> generarBuckets(List<Record> records,
        ...
        for (int i = 0; i < desiredBuckets; i++) {
            long bucketSize = baseSize + (i < rem ? 1 : 0);
